/**
 * QR Code Generator - 轻量版
 * 基于 Kazuhiko Arase 的 qrcode-generator
 */
...
QRCode.Util = {
  ...
  getBCHTypeInfo: function(data) {
    var d = data << 10;
    while (QRCode.Util.getBCHDigit(d) - QRCode.Util.getBCHDigit(QRCode.G15) >= 0) {
      d ^= (QRCode.G15 << (QRCode.Util.getBCHDigit(d) - QRCode.Util.getBCHDigit(QRCode.G15)));
    }
    return ((data << 10) | d) ^ QRCode.G15_MASK;
  },
  getBCHTypeNumber: function(data) {
    var d = data << 12;
    while (QRCode.Util.getBCHDigit(d) - QRCode.Util.getBCHDigit(QRCode.G18) >= 0) {
      d ^= (QRCode.G18 << (QRCode.Util.getBCHDigit(d) - QRCode.Util.getBCHDigit(QRCode.G18)));
    }
    return (data << 12) | d;
  },
...
QRCode.G15 = (1 << 10) | (1 << 8) | (1 << 5) | (1 << 4) | (1 << 2) | (1 << 1) | (1 << 0);
QRCode.G15_MASK = (1 << 14) | (1 << 12) | (1 << 10) | (1 << 4) | (1 << 1);
QRCode.G18 = (1 << 12) | (1 << 11) | (1 << 10) | (1 << 9) | (1 << 8) | (1 << 5) | (1 << 2) | (1 << 0);
...
for (var i = 8; i < 256; i++) {
  QRCode.Math.EXP_TABLE[i] = QRCode.Math.EXP_TABLE[i - 4] ^ QRCode.Math.EXP_TABLE[i - 5] ^ QRCode.Math.EXP_TABLE[i - 6] ^ QRCode.Math.EXP_TABLE[i - 8];
}
for (var i = 0; i < 255; i++) {
...
function createQRCodeForText(text, errorCorrectLevel) {
  var lastError = null;

  for (var t = 1; t <= 40; t++) {
    try {
      var qr = new QRCode(t, errorCorrectLevel);
      qr.addData(text);
      qr.make();
      return qr;
    } catch (e) {
...
function generateQRCode(text, size) {
  size = size || 256;
  var errorCorrectLevel = QRCode.ErrorCorrectLevel.M;
  var qr;

  try {
    qr = createQRCodeForText(text, errorCorrectLevel);
  } catch (e) {
    try {
      qr = createQRCodeForText(text, QRCode.ErrorCorrectLevel.L);
    } catch (e2) {
      var byteLength = getUtf8ByteLength(text);
      throw new Error('内容过长（约 ' + byteLength + ' 字节），超出二维码最大容量（约 2953 字节）');
    }
  }
  
  var moduleCount = qr.getModuleCount();
  var tileSize = Math.floor(size / moduleCount);
  var actualSize = tileSize * moduleCount;
